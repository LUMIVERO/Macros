...
public static class CitaviMacro
{
	...
	{
		//****************************************************************************************************************
		// CHANGE NUMBERING TYPE FOR ALL KNOWLEDGE ITEMS OF THE SELECTED REFERENCES
		// 1.1 -- 2018-05-29 added ".ToList()" to foreach loop on reference.Quotations to avoid "enumeration changed" errors 
		// 1.0 -- 2017-12-12
		//
		// This macro sets the numbering type to "Margin numbers" for all knowledge items that currently have another 
		// numbering type (like "Pages") specified.
		...
		foreach (Reference reference in references)
		{
			foreach (KnowledgeItem knowledgeItem in reference.Quotations.ToList())
            {
                ...
                try
                {
                    if (knowledgeItem.PageRange.NumberingType != NumberingType.Margin)
                    {
                        PageRange newPageRange = knowledgeItemPageRange.Update(NumberingType.Margin);    
